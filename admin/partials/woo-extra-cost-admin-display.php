...
class WC_Settings_Extra_Cost {
                            ...
                            <div id="accordion_product">
                               ...
                               echo '<p class="red paid_version_feature">' . __('( Enable only for paid version )', 'woocommerce-extra-cost') . '</p>';
                                ...
                                <div class="div_based_product_coupon">
                                    ...
                                    <input type="hidden" name="woo-extra-cost-nonce" id="woo-extra-cost-nonce" value="<?php echo wp_create_nonce( "woo-extra-cost-nonce" ); ?>" />
	...
	public function save_extra_cost() {
         ...
         */
		check_ajax_referer( 'woo-extra-cost-nonce', 'woo-extra-cost-nonce', false );

		$ec_product_sku_enable = isset($_POST['ec-product-sku-enable']) ? sanitize_text_field( $_POST['ec-product-sku-enable'] ) : 'no';
		...
		if (isset($_POST['save'])) {
			...
			if (isset($_POST['aec_charge_type']) && !empty($_POST['aec_charge_type'])) {
				...
				$ace_charge_type = sanitize_text_field( maybe_serialize($_POST['aec_charge_type']) );
				update_option('aec_charge_type', $ace_charge_type  );
			}
			$available_gateways = WC()->payment_gateways->get_available_payment_gateways();

			$ec_paymentgateway_enable = isset($_POST['ec-paymentgateway-enable']) ? sanitize_text_field( $_POST['ec-paymentgateway-enable'] ) : 'no';
			...
			if (isset($_POST['ddl_master_setting']) && !empty($_POST['ddl_master_setting'])) {
				update_option('extra_cost_master_setting',sanitize_text_field( $_POST['ddl_master_setting'] ) );
				
			}
			if (isset($_POST['single_unit_label']) && !empty($_POST['single_unit_label'])) {
				update_option('extra_cost_master_single_unit_label_setting',sanitize_text_field( $_POST['single_unit_label']) );
			...
			$ec_user_role_enable = isset($_POST['ec-user-role-enable']) ? sanitize_text_field( $_POST['ec-user-role-enable'] ) : 'no';
			$ec_country_enable = isset($_POST['ec-country-enable']) ? sanitize_text_field( $_POST['ec-country-enable'] ) : 'no';
			$ec_shipping_class_enable = isset($_POST['ec-shipping-class-enable']) ? sanitize_text_field( $_POST['ec-shipping-class-enable'] ) : 'no';
			$ec_cart_max_total_enable = isset($_POST['ec-cart-max-total-enable']) ? sanitize_text_field( $_POST['ec-cart-max-total-enable'] ) : 'no';
			...
			$length_based_country = isset($_POST['extra_cost_based_country_code']) ? sanitize_text_field( $_POST['extra_cost_based_country_code'] ) : '';
			...
			if (isset($_POST['ec_shipping_name']) && !empty($_POST['ec_shipping_name'])) {
				...
				$shipping_class_title_array = isset($_POST['ec_shipping_name']) ? array_map('sanitize_text_field', wp_unslash( $_POST['ec_shipping_name']) ) : array();
				$shipping_class_charge_array = isset($_POST['ec_shipping_charge']) ? array_map('absint',  wp_unslash($_POST['ec_shipping_charge']) ) : array();
		...
		/* --------------------------------------------------------------------------------------------------------------------------------------------- */
			...
			$ec_variable_product_enable = isset($_POST['ec-variable-product-enable']) ? sanitize_text_field( $_POST['ec-variable-product-enable'] ) : 'no';
			...
			$ec_product_enable = isset($_POST['ec-product-enable']) ? sanitize_text_field( $_POST['ec-product-enable'] ) : 'no';
			if (!empty($ec_product_enable) && $ec_product_enable != '') {
				update_option('wc_extra_cost_cart_product', $ec_product_enable);
			}

			$ec_product_category_enable = isset($_POST['ec-product-category-enable']) ? sanitize_text_field( $_POST['ec-product-category-enable'] ) : 'no';
			...
			$ec_product_tag_enable = isset($_POST['ec-product-tag-enable']) ? sanitize_text_field( $_POST['ec-product-tag-enable'] ) : 'no';
			if (!empty($ec_product_tag_enable) && $ec_product_tag_enable != '') {
				update_option('wc_extra_cost_cart_tag_product', $ec_product_tag_enable);
			}


			$ec_product_coupon_enable = isset($_POST['ec-product-coupon-enable']) ? sanitize_text_field( $_POST['ec-product-coupon-enable'] ) : 'no';
			...
			$length_based_product = isset($_POST['extra_cost_based_product_cost_title']) ? array_map( 'sanitize_text_field',wp_unslash( $_POST['extra_cost_based_product_cost_title'] ) ): '';
			...
			$length_based_product_cat = isset($_POST['extra_cost_based_product_category_title']) ? array_map( 'sanitize_text_field',wp_unslash( $_POST['extra_cost_based_product_category_title'] ) ): '';
